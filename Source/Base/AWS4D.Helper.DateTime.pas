unit AWS4D.Helper.DateTime;

interface

uses System.sysUtils, System.DateUtils;

type TAWS4DHelperDateTime = record helper for TDateTime
...
{ TAWS4DHelperDateTime }

function TAWS4DHelperDateTime.DateTimeToIso8601: string;
begin
  if Self = 0 then
    Result := ''
  else
...
function TAWS4DHelperDateTime.Format(ADateFormat: String): String;
begin
  result := FormatDateTime(ADateFormat, Self);
end;

function TAWS4DHelperDateTime.FormatYYYY_MM_DD: string;
begin
  result := Format('yyyy-MM-dd');
end;

procedure TAWS4DHelperDateTime.fromIso8601ToDateTime(AValue: String);
begin
  Self := Iso8601ToDateTime(AValue);
end;

function TAWS4DHelperDateTime.Iso8601ToDateTime(AValue: String): TDateTime;
var
